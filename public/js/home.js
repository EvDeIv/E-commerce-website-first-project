...
function createProductCards(data, parent) {
  //here paernt is for search product
  ...
  for (let i = 0; i < data.length; i++) {
    if (data[i].id != decodeURI(location.pathname.split("/").pop())) {
      ...
      <div class="product-image">
        ...
        <img src="${data[i].images[0]}" alt="product" class="product-thumb" onclick="location.href = '/product/${data[i].id}'" />
